...
public class SchedulerImpl implements Scheduler, ManagedProcessEventListener, ProcessLifecycleListener, AppStateListener {
    ...
    private static ManagedProcessHandler.Timeout stopTimeoutFor(ProcessId processId, AppSettings settings) {
        return switch (processId) {
            case ELASTICSEARCH ->
                HARD_STOP_TIMEOUT;
            case WEB_SERVER ->
                newTimeout(getStopTimeoutMs(settings, WEB_GRACEFUL_STOP_TIMEOUT), TimeUnit.MILLISECONDS);
            case COMPUTE_ENGINE ->
                newTimeout(getStopTimeoutMs(settings, CE_GRACEFUL_STOP_TIMEOUT), TimeUnit.MILLISECONDS);
            default ->
    ...
    /**
     * Tries to start the web leader process. If the process fails to start, the
     * web leader lock is released. If we would not release the lock then all
     * nodes would need to be stopped and restarted.
     */
    ...
    /**
     * Request for graceful stop then blocks until process is stopped. Returns
     * immediately if the process is disabled in configuration.
     *
     * @throws InterruptedException if {@link ManagedProcessHandler#hardStop()}
     * throws a {@link InterruptedException}
     */
    ...
    /**
     * Blocks until all processes are quickly stopped. Pending restart, if any,
     * is disabled.
     */
    @Override
    ...
    /**
     * This might be called twice: once by the state listener and once by the
     * stop/hardStop implementations. The reason is that if all process are
     * already stopped (may occur, eg., when stopping because restart of 1st
     * process failed), the node state won't be updated on process stopped
     * callback.
     */
    ...
    public boolean areAllProcessesOperational() {
        for (ProcessId processId : processesById.keySet()) {
            if (!appState.isOperational(processId, true)) {
                return false;
            }
        }
        return true;
