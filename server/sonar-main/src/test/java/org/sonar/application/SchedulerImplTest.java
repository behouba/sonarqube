...
public class SchedulerImplTest {
    ...
    @Test
    public void areAllProcessesOperational_returns_true_when_all_processes_are_operational() throws InterruptedException {
        TestAppSettings settings = new TestAppSettings();
        SchedulerImpl underTest = newScheduler(settings, false);
        underTest.schedule();

        // Elasticsearch devient opérationnel
        processLauncher.waitForProcess(ELASTICSEARCH).signalAsOperational();
        waitForAppStateOperational(appState, ELASTICSEARCH);

        // Web Server devient opérationnel
        processLauncher.waitForProcess(WEB_SERVER).signalAsOperational();
        waitForAppStateOperational(appState, WEB_SERVER);

        // Compute Engine devient opérationnel
        processLauncher.waitForProcess(COMPUTE_ENGINE).signalAsOperational();
        waitForAppStateOperational(appState, COMPUTE_ENGINE);

        assertThat(underTest.areAllProcessesOperational()).isTrue();
    }

    @Test
    public void areAllProcessesOperational_returns_false_when_not_all_processes_are_operational() throws InterruptedException {
        TestAppSettings settings = new TestAppSettings();
        SchedulerImpl underTest = newScheduler(settings, false);
        underTest.schedule();
        ...
        // Compute Engine n'est pas encore opérationnel
        assertThat(underTest.areAllProcessesOperational()).isFalse();
    }
